classdef arduino < handle
    ...
    methods
        ...
        function Faststeps(a, n, Motor, dir)
            %%%%%%%%%%%%%%%%%%%%%%%%% ARGUMENT CHECKING %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
            % ?bernommen aus digitalWrite() und modifiziert
            % check arguments if a.chkp is true
            if a.chkp,
                
                % check nargin
                if n > 65535
                    error('Number of steps must be below 65536');
                end
                if nargin~=4,
            ...
            % Clear serial Input
            if a.aser.BytesAvailable ~= 0
                fread(a.aser,a.aser.BytesAvailable);
            end
            if strcmpi(get(a.aser,'Port'),'DEMO'),
                % handle demo mode
                ...
                % n in Zahl mit Basis 256 umwandeln
                n1 = n/256;
                n0 = mod(n,256);
                % Pin und Richtung festlegen
                switch Motor
                    case 1
                        if dir > 0
                            motdir = 99;
                ...
                fwrite(a.aser,['X' motdir n1 n0],'uchar');  
                while (a.aser.BytesAvailable == 0) % Warten bis Arduino eine Antwort schickt
                    pause(0.1)
                end
                pause(0.2);                
                fread(a.aser,a.aser.BytesAvailable);   
